...
+ (CKHTTPConnection *)connectionWithRequest:(NSURLRequest *)request settings:(NSDictionary *)settings delegate:(id <CKHTTPConnectionDelegate>)delegate
{
    return [[self alloc] initWithRequest:request settings:settings delegate:delegate];
}

- (id)initWithRequest:(NSURLRequest *)request settings:(NSDictionary *)settings delegate:(id<CKHTTPConnectionDelegate>)delegate
{
...
- (void)start
{
    ...
    if([[[url scheme] lowercaseString] isEqualToString:@"https"]) {
      ...
      NSString *tlsVersionOption = [self.settings valueForKey:@"tlsver"];
        if ([tlsVersionOption length]) {
            CFDictionarySetValue(sslOptions, kCFStreamSSLLevel, (__bridge const void *)(tlsVersionOption));
            setSSLOption = YES;
        }
        else {
      ...
      if ([URL.absoluteString rangeOfString:@"https://"].location == 0) {
          ...
          for (NSString *whitelistHost in self.settings[@"kCKWhiteListDomains"]) {
